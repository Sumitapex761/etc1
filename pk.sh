...
echo "📁 Setting up Root CA structure..."
...
echo "📁 Setting up Sub CA structure..."
...
echo "📥 Copying Sub CA cert to Sub CA directory..."
cp subca/certs/subca.cert.pem "$SUBCA_DIR/certs/"

echo "👤 Creating user certificate request..."
mkdir -p "$USER_DIR"
cd "$USER_DIR"
...
echo "📥 Copying user CSR to Sub CA..."
...
echo "📦 Copying chain files to user folder and building certificate chain..."
cd "$USER_DIR"
cp "$SUBCA_DIR/certs/subca.cert.pem" .
cp "$ROOTCA_DIR/certs/ca.cert.pem" .
cat shuhari.local.cert.pem subca.cert.pem ca.cert.pem > shuhari.local.cert.chain.pem

echo "✅ PKI setup complete. All files are in $USER_DIR"
tree "$USER_DIR"
